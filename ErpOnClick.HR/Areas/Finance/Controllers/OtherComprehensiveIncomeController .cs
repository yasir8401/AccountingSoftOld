    ...
    public class OtherComprehensiveIncomeController : Controller
    {
        ...
        public OtherComprehensiveIncomeViewModel GenerateReportForOtherComprehensiveIncome()
        {
            ...
            foreach (var accountVouchers in voucherLines)
            {
                ...
                else if (accountDetail.ParentAccount.AccountNameEn.Contains("Other comprehensive income for the period") || accountDetail.ParentAccount.ParentAccount.AccountNameEn.Contains("Other comprehensive income for the period") || accountDetail.ParentAccount.ParentAccount.ParentAccount.AccountNameEn.Contains("Other comprehensive income for the period") || accountDetail.ParentAccount.ParentAccount.ParentAccount.ParentAccount.AccountNameEn.Contains("Other comprehensive income for the period"))
                {
                    otherComprehensiveIncomeViewModel.OCI_2021 = Convert.ToDecimal(accountVouchers.Where(c => c.CreatedDate.Value.Year == 2021).Sum(c => c.Credit));
                    otherComprehensiveIncomeViewModel.OCI_2020 = Convert.ToDecimal(accountVouchers.Where(c => c.CreatedDate.Value.Year == 2020).Sum(c => c.Credit));
                }
